    ...
    <div class="container">
      <div class="path-links text-start pt-4">
        <h5 class="links-frame" dir="auto">
          ...
          /
          <nuxt-link class="path-link" :to="localePath(`/services`)">{{
            $t("services")
          }}</nuxt-link>
          /
      ...
      <div class="main-content">
        <div class="content-body">
          ...
          <div class="description">
            <div class="section">
              <div
                v-if="dataResult?.productImageUrl"
                class="img-frame contain"
                :class="rtl()"
              >
                <img :src="cImg(dataResult.productImageUrl)" :alt="useAlt(dataResult.productSeos,1)" />
              </div>
              <div class="des" v-html="useDes(dataResult)"></div>
            </div>
            <div class="section">
              <div
                v-if="dataResult?.productImageUrl1"
                class="img-frame contain"
                :class="rtl()"
              >
                <img :src="cImg(dataResult.productImageUrl1)" :alt="useAlt(dataResult.productSeos,1)" />
              </div>
              <div class="des" v-html="useDes1(dataResult)"></div>
            </div>
            <div class="section">
              <div
                v-if="dataResult?.productImageUrl2"
                class="img-frame contain"
                :class="rtl()"
              >
                <img :src="cImg(dataResult.productImageUrl2)" :alt="useAlt(dataResult.productSeos,2)" />
              </div>
              <div class="des" v-html="useDes2(dataResult)"></div>
            </div>
            <div class="section">
              <div
                v-if="dataResult?.productImageUrl3"
                class="img-frame contain"
                :class="rtl()"
              >
                <img :src="cImg(dataResult.productImageUrl3)" :alt="useAlt(dataResult.productSeos,3)" />
              </div>
...
try {
  ...
  if (serviceError.value || !serviceData.value?.data) {
    throw new Error(serviceError.value?.message || t("errors.serviceNotFound"));
  }
  ...
  if (err.message.includes("404") || err.message.includes("not found")) {
    // Handle 404 by either:
    // 1. Showing error page (recommended)
    error.value = t("errors.serviceNotFound");

    // OR 2. Redirecting to 404 page
    // await router.replace(localePath('/404'));

    // OR 3. Using Nuxt's showError (only works in server-side)
    // throw createError({ statusCode: 404, message: t('errors.serviceNotFound') });
  }
