<template>
  <div class="mb-1 position-relative">
    <Field :name="name" v-slot="{ field, meta }">
      <label class=" w-100 text-start" :for="id" :style="`color: #${color}`" :class="`form-label mb-3   ${!isEn() ? 'rtl' : '' }`" v-if="label">
        {{ label }} 
      ...
      <div class="input-frame">
        ...
        <span class="show-hide-password pcc" :class="rtl()" v-if="activePasswordEye">
          <icon id="show_password" name="mdi:eye" size="20px" @click="showPassword"/>
          <icon id="hide_password" class="d-none" name="mdi:eye-closed" size="20px" @click="hidePassword"/>
